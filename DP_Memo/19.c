...
/*
Question 19: Skyline Problem
Problem Description:
You are given N buildings, where each building is represented as three integers: left, right,
height.
Each building spans from coordinate left to right (exclusive of right) and has a constant height.
Your task is to compute the skyline formed by these buildings using a Divide and Conquer
approach.
The skyline is defined as a list of key points (x, height) such that:
● At each key point, the height of the skyline changes
● The output must be sorted by x-coordinate
● If multiple buildings overlap, the skyline height is the maximum height at that point
*/
